        ...
        <>
            ...
            <Section>
                <div className="text-center mb-16">
                    <h2 className="text-3xl md:text-4xl font-poppins font-bold">Key Features</h2>
                    <p className="mt-4 max-w-2xl mx-auto text-lg text-slate-400">An entire top-of-funnel sales team, in one AI agent.</p>
                </div>
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
                    {features.map((feature, i) => (
                        <div key={i} className="p-6 bg-slate-900/50 rounded-3xl border border-slate-800">
            ...
            <Section className="bg-navy-light">
                 <div className="text-center mb-16">
                    <h2 className="text-3xl md:text-4xl font-poppins font-bold">The ROI of AI Sales Automation</h2>
                     <p className="mt-4 max-w-2xl mx-auto text-lg text-slate-400">JillJill doesn't just save time—it drives significant, measurable financial results.</p>
                </div>
                <div className="overflow-x-auto">
                    <table className="w-full text-left border-collapse">
                        <thead>
                            <tr className="border-b border-slate-700">
                                <th className="p-4 text-sm font-semibold uppercase text-slate-400">Metric</th>
                                <th className="p-4 text-sm font-semibold uppercase text-cyan text-center">JillJill AI Agent</th>
                                <th className="p-4 text-sm font-semibold uppercase text-slate-500 text-center">Human SDR</th>
                            </tr>
                        </thead>
                        <tbody>
            ...
            <Section>
                <div className="text-center mb-16">
                    <h2 className="text-3xl md:text-4xl font-poppins font-bold">Implementation Workflow</h2>
                    <p className="mt-4 max-w-2xl mx-auto text-lg text-slate-400">Our structured 6-step process ensures a smooth and successful deployment.</p>
                </div>
                 <div className="max-w-4xl mx-auto grid grid-cols-1 md:grid-cols-2 gap-x-12 gap-y-8">
                    {processSteps.map((step, i) => (
                         <div key={i} className="flex items-center p-3 pr-5 bg-slate-900 rounded-full border border-slate-800 transition-all duration-300 hover:border-cyan/70 hover:shadow-lg hover:shadow-cyan/10 transform hover:-translate-y-1">
