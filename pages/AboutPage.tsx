...
const AboutPage: React.FC = () => {
    ...
    return (
        <>
                ...
                <div className="max-w-7xl mx-auto">
                    <div className="grid grid-cols-1 lg:grid-cols-2 gap-12 items-center">
                        <div className={`${storyVisible ? 'animate-fade-in-up' : 'opacity-0'}`}>
                            ...
                            <p className="text-slate-400 leading-relaxed">Today, we are a dedicated partner for businesses in specialized industries, providing the strategic guidance and technical implementation needed to thrive in the digital age. Our integrated approach ensures that every piece of your digital presence—from your personal brand to your sales process—works together as a powerful, cohesive growth engine.</p>
                        </div>
                        <div className={`bg-slate-900/50 p-8 rounded-4xl border border-slate-800 ${storyVisible ? 'animate-fade-in-up' : 'opacity-0'}`} style={{ animationDelay: '200ms' }}>
                            ...
                            <div className="grid grid-cols-2 sm:grid-cols-3 gap-6">
                                {focusAreas.map((area, i) => (
                                    <div key={area.name} className={`flex flex-col items-center text-center ${storyVisible ? 'animate-fade-in-up' : 'opacity-0'}`} style={{ animationDelay: `${300 + i * 100}ms` }}>
                                        <div className="w-16 h-16 mb-2 flex items-center justify-center bg-slate-800 rounded-full">{React.cloneElement(area.icon, {size: 28})}</div>
            ...
            <section ref={valuesRef} className="bg-navy-light py-16 md:py-24 px-4 sm:px-6 lg:px-8">
                <div className="max-w-7xl mx-auto">
                    <div className="text-center mb-16">
                        <h2 className={`text-3xl md:text-4xl font-poppins font-bold ${valuesVisible ? 'animate-fade-in-up' : 'opacity-0'}`}>Our Values</h2>
                        <p className={`mt-4 max-w-2xl mx-auto text-lg text-slate-400 ${valuesVisible ? 'animate-fade-in-up' : 'opacity-0'}`} style={{ animationDelay: '150ms' }}>The principles that guide every strategy we build and every partnership we form.</p>
                    </div>
                    <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-8">
                        {values.map((value, i) => (
                            <div key={i} className={`p-6 text-center bg-slate-900 rounded-3xl border border-slate-800 ${valuesVisible ? 'animate-fade-in-up' : 'opacity-0'}`} style={{ animationDelay: `${300 + i * 100}ms` }}>
                               <div className="w-16 h-16 mx-auto mb-4 flex items-center justify-center bg-slate-800 rounded-2xl text-cyan">{value.icon}</div>
                    ...
                    <div className="flex flex-wrap justify-center gap-8">
                        {teamMembers.map((member, i) => (
                            <div key={i} className={`text-center ${teamVisible ? 'animate-fade-in-up' : 'opacity-0'}`} style={{ animationDelay: `${150 + i * 100}ms` }}>
                                <img src={member.imageUrl} alt={`Photo of ${member.name}`} className="w-48 h-48 mx-auto rounded-full object-cover border-4 border-slate-700 mb-4" />
